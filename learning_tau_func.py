...
img = f_images[0]
y = ray_transform_module(img)
x = fbp_operator_module(y)
odl_grad = adjoint_operator_module(ray_transform_module(x)-y)
print(torch.norm(odl_grad))

x = x.requires_grad_(True)
fit = 0.5*(torch.norm(ray_transform_module(x)-y,p=2)**2)
grad_fit = torch.autograd.grad(fit, x)[0]
print(torch.norm(grad_fit))

x = fbp_operator_module(y)


t = torch.tensor([3,2], dtype=torch.float32)
r = torch.tensor([3,5], dtype=torch.float32)
matrix = torch.tensor([[1, 2],
                       [1, 2]], dtype=torch.float32)
t = t.requires_grad_(True)
fit = 0.5*torch.norm(torch.matmul(matrix, t)-r,p=2)**2
grad_fit = torch.autograd.grad(fit, t)[0]
print(grad_fit)
print(torch.matmul(matrix.t(), torch.matmul(matrix, t)-r))
...
adjoint_operator_module(ray_transform_module(x_stars[0,:,:])-g_sinograms[0,:,:,:])
...
def train_network(net, f_images, g_sinograms, f_rec_images, f_test_rec_images, \
    ...
    for i in range(n_train): 
        ...
        # else:
        #     if i % 1000 == 0:
        #         optimizer = optim.Adam(LGS_parameters, lr=0.0001)
        #         n_iter+=1
        #         print('NUMBER OF ITERS:', n_iter)
        ...
        if i % 1000 == 0:
            ...
            #outs2, step_len, outs2_list = net(f_test_rec_images, g_test_sinograms, n_iter=n_iter)
            ...
            #     plt.figure()
        # if i % 1000 == 0:
        #     torch.save(net.state_dict(), 'TAU_SUPERVISED.pth')
        #     print('saved')      
