...
def train_network(net, ys, n_train=50000, batch_size=4):
    ...
    optimizer = optim.AdamW(tau_parameters, lr=0.0005) #betas = (0.9, 0.99)
    ...
    for i in range(n_train): 
        ...
        #     if num_five > 5000:
        #         if i%1000 == 0:
        #             print('Iteration from:', 0, 'to', 5)
        #             print(_)
        #             print('LOSS:', loss.item())
        #         for k in range(min([10, 1+int(num_five/100000)])):
        #             xk = outs
        #             optimizer.zero_grad()
        #             outs, _, outs_list = net(xk, g_batch, n_iter=k)
        #             loss = f(outs, g_batch, model, alpha)
        #             loss.backward()
        #             torch.nn.utils.clip_grad_norm_(tau_parameters, max_norm=1.0, norm_type=2)
        #             optimizer.step()
        #             if i%1000 == 0:
        #                 print('Iteration from:', 5*(k+1), 'to', 5*k+10)
        #                 print(_)
        #                 print('LOSS:', loss.item())
        #                 torch.save(net.state_dict(), 'BLURRING_TAU_UNSUPERVISED_10.pth')
        #                 print('saved')   
