...
def train_network(net, f_images, g_sinograms, f_rec_images, f_test_rec_images, \
    ...
    for i in range(n_train):
        ...
        if i <500:
            #print(torch.mean((g_batch - ray_transform_module(outs))**2))
            #print(torch.norm((g_batch - ray_transform_module(outs))**2))
            loss = torch.mean((g_batch - ray_transform_module(outs))**2) + reg(outs, .25)#loss_train(f_batch, outs)
        else:
            loss = f(outs, g_batch, ray_transform_module, alpha=0.0)#loss_train(f_batch, outs)
