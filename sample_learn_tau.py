...
import numpy as np
from LGS_train_module import get_images, geometry_and_ray_trafo, LGD, LGD2, TauModelSample
...
tau_network = TauModelSample(adjoint_operator_module, ray_transform_module, lambda x: reg(x, 0.0001), in_channels=6, out_channels=1).to(device)
...
def train_network(net, x_list, x_stars, x_list_test, x_stars_test, n_train=500000, batch_size=4):
    ...
    for i in range(n_train):
        ...
        k_iter = random.choice(range(len(x_list_new)))
        xk = x_list_new[k_iter].to(torch.float32).unsqueeze(0).detach()
        if k_iter > 0:
            xkm1 = x_list_new[k_iter-1].to(torch.float32).unsqueeze(0).detach()
        else:
            xkm1 = x_list_new[k_iter].to(torch.float32).unsqueeze(0).detach()
        
        net.train()
        
        optimizer.zero_grad()

        outs, _, outs_list = net(xk, xkm1, y, n_iter=n_iter)
        ...
        if i % 1000==0:
            ...
            print('PSNR:', psnr(train_loss), train_loss, k_iter)
        ...
        if i % 1000 == 0:
            torch.save(net.state_dict(), 'INTERMEDIATE_SAMPLE_TAU.pth')
            print('saved')      
