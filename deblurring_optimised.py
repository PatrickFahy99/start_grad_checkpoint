...
def train_network(net, ys, n_train=int(1e10), batch_size=1):
    ...
    num_iterations_in_backprop = 100
    ...
    for i in range(n_train): 
        ...
        for _ in range(number_of_starting_iter_batches):
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                outs, tau, outs_list = net(inpt, g_batch, n_iter= num_iterations_in_backprop)
        ...
        if i % (train_iters_each_iteration_batch//10) == 0:
            if number_of_starting_iter_batches == 0:
                ...
                torch.save(net.state_dict(), f'models/BLURRING_TAU_UNSUPERVISED_GREEDY_{num_iterations_in_backprop}_{number_of_starting_iter_batches}_{train_iters_each_iteration_batch}.pth')
                print('saved')
                print(f'models/BLURRING_TAU_UNSUPERVISED_GREEDY_{num_iterations_in_backprop}_{number_of_starting_iter_batches}_{train_iters_each_iteration_batch}.pth')
